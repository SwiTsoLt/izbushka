...
export class UserCardComponent implements OnInit, OnChanges {
  ...
  public readonly current_user$: Observable<User> = this.store.select(
    selectUser as never,
  );

  private readonly areaList$: Observable<LocationArea[]> = this.store.select(
      ...
      .pipe(
        map(([areaList, regionList]) => {
          const userArea = areaList.find(
            (area) => area._id === this.user?.location.area,
          );
          const userRegion = regionList.find(
            (region) => region._id === this.user?.location.region,
          );
